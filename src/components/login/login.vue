<template>
  <div class="login">
    ...
    <group>
      <x-input title="账号" v-model='user_name' type="text" placeholder="请输入账号" required></x-input>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(['login']),
      submit () {
        if (this.remember) { localStorage.setItem('user_name', this.user_name) } else { delete localStorage.user_name }
        const user = {user_name: this.user_name, password: this.password, requestType: 'login'}
        this.login(user)
      }
    }
  }
